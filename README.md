...
### 🌟 兩種自選開始模式
1. **🎲 模式一：隨機全台樞紐出發**（快速開始，在全台 8 大核心交通樞紐隨機降落，雙北各區隨機散布待援小熊）。
2. **📍 模式二：自訂台灣地址/地標出發**（支援輸入「台北市信義區...」、「新北板橋...」、「台中勤美...」或自家附近地址，系統自動精確定位並在周邊生成待援小熊與最近捷運/YouBike站點！）。

### 🕹️ 鍵盤操作速查表
- `W` / `A` / `S` / `D` 或 `↑` `↓` `←` `→`：上下左右移動
- `Z 鍵`：**上下大眾運具切換**（在運具上加速 4~30 倍 / 步行慢速探索）
- `Space 空白鍵` / `Enter`：靠近小熊時「抱起救援」、抵達醫院時「送入急診」
- `1` 步行 / `2` YouBike (18km/h) / `3` 捷運 (55km/h) / `4` 幹線公車 (30km/h) / `5` 高鐵 (220km/h)
